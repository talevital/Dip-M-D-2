"""
API ETL avec authentification intégrée directement
...
# Imports d'authentification intégrés
...
def create_app() -> FastAPI:
    ...
    # Routes d'authentification intégrées directement
    @app.post("/auth/register", response_model=UserResponse)
    def register_user(user: UserCreate, db = Depends(get_auth_db)):
    ...
    def login_user(user_credentials: UserLogin, request: Request, db = Depends(get_auth_db)):
    ...
    def get_user_profile(current_user: User = Depends(get_current_active_user), db = Depends(get_auth_db)):
    ...
    def export_file(
        ...
        """Export simple d'un fichier (protégé)"""
        with get_session() as session:
            uf = session.get(UploadedFile, file_id)
            if not uf:
                raise HTTPException(status_code=404, detail="File not found")
            ...
            try:
                ...
                if format.lower() == "json":
                    return {
                        ...
                        "metadata": {
                            "original_filename": uf.original_name,
                            "row_count": len(df),
                            "col_count": len(df.columns),
                            "columns": list(df.columns),
                            "file_size": uf.size_bytes,
                            "created_at": uf.created_at.isoformat()
                        }
                    }
                    ...
                    return Response(
                        content=csv_content,
                        media_type="text/csv",
                        headers={"Content-Disposition": f"attachment; filename={uf.original_name}"}
                    )
                else:
    ...
    # Routes de scoring des pays
    @app.get("/api/scoring/years")
    async def get_scoring_years(current_user: User = Depends(get_current_user)):
        """Récupère les années disponibles pour le scoring"""
        try:
            from etl.ml.country_scoring import CountryScoringEngine
    ...
    async def get_scoring_countries(current_user: User = Depends(get_current_user)):
        """Récupère les pays disponibles pour le scoring"""
        try:
            from etl.ml.country_scoring import CountryScoringEngine
            engine = CountryScoringEngine()
    ...
    async def calculate_scoring(
        ...
        """Calcule les scores des pays selon les paramètres fournis"""
        try:
            ...
            from etl.ml.country_scoring import CountryScoringEngine
            engine = CountryScoringEngine()
            result = engine.calculate_scoring(year=year, weights=weights, countries=countries)
            return result
        except Exception as e:
